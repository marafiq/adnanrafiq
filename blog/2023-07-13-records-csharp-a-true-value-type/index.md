...
Now if you do the following `` will also print true on the console. Because both the objects have same values. But `record` is a reference type like `class`.
...
Tradeoff: If your requirement is that value equality should only compare some properties to determine equality then manual `class` implementation have some advantage. 
But you can `override` the semantics of record value equality in the below example. Also, notice you can have methods like you can have in classes.  
...
A record declared using positional property syntax is immutable. You can not set the value of its any property because the setter generated by the compiler is `init` only which can only set value within the constructor.
If you try to the following `var n=new Name("A","Z"); n.FirstName="C"; "` you will get a compiler error. Since it is immutable, it is thread safe since no one can change its value once created.

But if you like to make it mutable, you can certainly do that by declaring the property setter accordingly yourself just like classes, but you should avoid this, and may be your use case does need `class` instead of `record`.
...
// highlight-end
```
Along with nice syntax it is useful in scenarios where you would like to assignment object property value to a local variable to operate on it.
...
`record` does support inheritance like `class`. Before going the inheritance consider your use case, do you really need inheritance with value equality semantics, if yes. Sure.  
